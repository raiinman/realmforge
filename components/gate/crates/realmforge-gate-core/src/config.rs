...
/// Runtime configuration for a Realmforge service.
///
...
pub struct Config {
    ...
    /// `postgres://realmforge:realmforge@localhost:5432/realmforge`. Required.
...
mod tests {
    ...
    fn valid_with_only_required_key_applies_defaults() {
        let cfg = Config::from_pairs([(KEY_DATABASE_URL, "postgres://localhost/realmforge")])
            .expect("valid config");
        assert_eq!(cfg.database_url, "postgres://localhost/realmforge");
    ...
    fn unknown_keys_are_ignored() {
        let cfg = Config::from_pairs([
            (KEY_DATABASE_URL, "postgres://localhost/realmforge"),
            ("UNRELATED", "noise"),
        ])
        .expect("valid config");
        assert_eq!(cfg.database_url, "postgres://localhost/realmforge");
    ...
    fn all_keys_override_defaults() {
        let cfg = Config::from_pairs([
            ...
            (KEY_SIGNING_KEY_PATH, "/etc/realmforge/key.pem"),
        ])
        .expect("valid config");
        ...
        assert_eq!(
            cfg.signing_key_path,
            PathBuf::from("/etc/realmforge/key.pem")
        );
    }
    ...
    fn tls_paths_are_optional_and_parse() {
        let cfg = Config::from_pairs([
            (KEY_DATABASE_URL, "postgres://localhost/realmforge"),
            (KEY_TLS_CERT_PATH, "/etc/realmforge/tls/cert.pem"),
            (KEY_TLS_KEY_PATH, "/etc/realmforge/tls/key.pem"),
        ])
        .expect("valid config");
        assert_eq!(
            cfg.tls_cert_path,
            Some(PathBuf::from("/etc/realmforge/tls/cert.pem"))
        );
        assert_eq!(
            cfg.tls_key_path,
            Some(PathBuf::from("/etc/realmforge/tls/key.pem"))
        );

        let plain = Config::from_pairs([(KEY_DATABASE_URL, "postgres://localhost/realmforge")])
            .expect("valid config");
