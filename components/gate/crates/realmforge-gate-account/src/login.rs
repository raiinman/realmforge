...
use realmforge_gate_core::srp::{self, ServerSession};
    ...
    match verify_srp_proof(&state, &form).await {
        Ok(account_id) => {
            ...
            if !redirect_url.contains("authorize") {
                ...
                let _ = realmforge_gate_db::sessions::insert(&state.pool, &session).await;
                set_cookie(
                    headers,
                    "SESSIONID",
...
async fn verify_srp_proof(state: &Arc<AppState>, form: &SrpProofForm) -> Result<i64, AccountError> {
    ...
    realmforge_gate_observability::record_srp_duration(start);
