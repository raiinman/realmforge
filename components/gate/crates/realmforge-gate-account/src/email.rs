...
//! layout with logo → divider → body → footer → bottom bar. Realmforge branding
//! replaces Blizzard/Battle.net.
//!
//! The Realmforge emblem is embedded as a CID inline image (not remote-hosted):
...
//! `docs/assets/realmforge-logo-monochrome.svg`).
...
const LOGO_LIGHT_CID: &str = "realmforge-logo-light"; // black glyph, light theme
const LOGO_DARK_CID: &str = "realmforge-logo-dark"; // white glyph, dark theme
...
pub async fn send_welcome_email(
    ...
    let verify_ticket = realmforge_gate_core::encrypted_ticket::seal(account_id, signing_key_pem)
        .unwrap_or_else(|e| {
...
fn header_html(battletag: Option<&str>, locale: &str) -> String {
    ...
    let logo = format!(
        r#"<img class="logo-light" src="cid:{light}" width="120" height="120"
             alt="Realmforge" style="width:120px;height:120px;border:0;line-height:1"/>
...
fn footer_html(server_url: &str, locale: &str, _email: &str) -> String {
    let signoff = match locale {
        "deDE" => "Vielen Dank,<br><b>Realmforge</b>",
        "koKR" => "감사합니다,<br><b>Realmforge</b>",
        _ => "Thank you,<br><b>Realmforge</b>",
    };
...
<table width="600" border="0" cellpadding="0" cellspacing="0" align="center"
  ...
  <tr>
    <td style="padding:24px 40px;text-align:center;font-size:11px;
               ...
               background:var(--footer-bg);
               {EMAIL_STYLES}">
      Realmforge &middot; <a href="https://wowemulation.dev/" style="color:#b0b8c8;color:var(--footer-text);text-decoration:underline">WoW Emulation</a> &middot; Digital Preservation<br>
      <br>
      <a href="https://github.com/raiinman/realmforge" style="color:#b0b8c8;color:var(--footer-text);text-decoration:underline">Realmforge</a> is free software under the AGPL 3.0 license.<br>
      <a href="{server_url}" style="color:#b0b8c8;color:var(--footer-text);text-decoration:none">
        Realmforge Account Management</a> &middot;
      <a href="{server_url}" style="color:#b0b8c8;color:var(--footer-text);text-decoration:none">
        {support}</a>
    </td>
  </tr>
...
fn localized_plain(locale: &str, to_email: &str, verify_url: &str, server_url: &str) -> String {
    match locale {
        "deDE" => format!(
            "Hallo,\n\nVielen Dank, dass Sie ein Realmforge-Konto erstellt haben!\n\n\
        ...
        _ => format!(
            "Hello,\n\nThank you for creating a Realmforge account!\n\n\
...
fn en_body(
    ...
    let subject = "Realmforge Account - Verify Your Email".into();
    let body = format!(
        "Hello,<br><br>\
         Thank you for creating a <b>Realmforge</b> account!<br><br>\
         The following <b>Realmforge</b> account has been created:<br><br>\
         <b>{to_email}</b><br><br>\
         ...
         at any time by visiting the Realmforge Account Management page."
    );
...
fn de_body(
    ...
    let subject = "Realmforge Konto - E-Mail-Adresse bestätigen".into();
    let body = format!(
        "Hallo,<br><br>\
         Vielen Dank, dass Sie ein <b>Realmforge</b>-Konto erstellt haben!<br><br>\
         Das folgende <b>Realmforge</b>-Konto wurde erstellt:<br><br>\
         <b>{to_email}</b><br><br>\
         ...
         jederzeit auf der Realmforge Kontoverwaltungsseite verwalten."
    );
...
fn ko_body(
    ...
    let subject = "Realmforge 계정 - 이메일 인증".into();
    let body = format!(
        "안녕하세요,<br><br>\
         <b>Realmforge</b> 회원으로 가입해 주셔서 감사합니다!<br><br>\
         다음과 같은 <b>Realmforge</b> 계정을 만드셨습니다:<br><br>\
         <b>{to_email}</b><br><br>\
         ...
         Realmforge 계정 관리</a> 페이지에서 언제든지 계정을 관리할 수 있습니다."
    );
...
mod tests {
    ...
    fn email_renders() {
        ...
        assert!(subject.contains("Realmforge"));
        ...
        assert!(
            html.contains("cid:realmforge-logo-light"),
            "missing light cid"
        );
        assert!(
            html.contains("cid:realmforge-logo-dark"),
            "missing dark cid"
        );
