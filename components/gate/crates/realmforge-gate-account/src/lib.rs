...
use realmforge_gate_core::srp::{Group, ServerSession};
...
pub fn router(state: std::sync::Arc<AppState>) -> Router {
    Router::new()
        ...
        .route(
            "/static/realmforge-logo-light.png",
            get(static_realmforge_logo_light),
        )
        .route(
            "/static/realmforge-logo-dark.png",
            get(static_realmforge_logo_dark),
        )
...
pub enum AccountError {
    ...
    Database(#[from] realmforge_gate_db::DbError),
...
async fn sso_redirect(
    ...
    let account_id = realmforge_gate_core::encrypted_ticket::open(token, &state.signing_key_pem)
...
/// `GET /static/realmforge-logo-light.png` — black-glyph emblem for light
/// theme surfaces.
async fn static_realmforge_logo_light() -> impl IntoResponse {
    let png: &'static [u8] = include_bytes!("../static/realmforge-logo-light.png");
    (
        axum::http::StatusCode::OK,
...
/// `GET /static/realmforge-logo-dark.png` — white-glyph emblem for dark
/// theme surfaces.
async fn static_realmforge_logo_dark() -> impl IntoResponse {
    let png: &'static [u8] = include_bytes!("../static/realmforge-logo-dark.png");
    (
        axum::http::StatusCode::OK,
        [(axum::http::header::CONTENT_TYPE, "image/png")],
        png,
