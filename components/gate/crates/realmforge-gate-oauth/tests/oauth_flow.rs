...
use realmforge_gate_oauth::{oauth_state, router};

const TEST_KEY_PEM: &str = include_str!("../../realmforge-gate-core/tests/data/test_key_pkcs8.pem");
...
async fn setup() -> Option<Arc<realmforge_gate_oauth::OAuthState>> {
    let url = db_url()?;
...
async fn register_account(pool: &sqlx::PgPool, email: &str) -> i64 {
    let group = realmforge_gate_core::srp::Group::battle_net_v2();
    let _ = sqlx::query("DELETE FROM accounts WHERE email = $1")
        .bind(email)
        .execute(pool)
        .await;
    realmforge_gate_account::registration::register(pool, &group, email, "password123")
        .await
        .expect("register")
}

async fn mint_ticket(pool: &sqlx::PgPool, account_id: i64) -> String {
    realmforge_gate_account::ticket::mint_login_ticket(pool, account_id)
        .await
        .expect("mint ticket")
}
...
async fn token_exchange_grant() {
    ...
    let bgs_jwt =
        realmforge_gate_core::jwt::sign(&claims, state.signing_keys.active()).expect("sign jwt");
...
async fn authorize_two_hop(state: &Arc<realmforge_gate_oauth::OAuthState>, url: &str) -> String {
    let app = router(state.clone());
