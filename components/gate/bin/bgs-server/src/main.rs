...
use tavern_bgs::frame::{parse_frame, serialize_frame};
use tavern_bgs::method_id;
use tavern_bgs::service_hash;
use tavern_bgs::{
...
pub struct BgsSession {
    ...
    pub game_accounts: Vec<tavern_bgs::GameAccountHandle>,
    pub game_account_info: Vec<GameAccountInfo>,
    pub licenses: Vec<tavern_bgs::AccountLicense>,
    pub per_game_account_licenses: std::collections::HashMap<i64, Vec<tavern_bgs::AccountLicense>>,
...
fn kick_existing_session(state: &Arc<BgsState>, account_id: u64, new_session_id: &str) {
        ...
        && let Some(tx) = state.push_channels.get(&old)
    {
        let dc = Header {
            ...
            token: 0,
            service_hash: Some(tavern_bgs::service_hash::CONNECTION_SERVICE),
            is_response: Some(false),
            ..Default::default()
        };
        let body = DisconnectNotification {
            error_code: tavern_bgs::result_code::ERROR_SESSION_DUPLICATE,
...
fn broadcast_boom(state: &Arc<BgsState>) {
    ...
    let body = DisconnectNotification {
        error_code: tavern_bgs::result_code::ERROR_SERVER_SHUTTING_DOWN,
...
fn dequeue_next(state: &Arc<BgsState>) {
    ...
    if let Some(entry) = entry {
        ...
        let end_header = Header {
            ...
            method_id: Some(em),
            token: 0,
            service_hash: Some(tavern_bgs::service_hash::AUTHENTICATION_CLIENT_V1),
            is_response: Some(false),
            ..Default::default()
        };
...
fn queue_push_params(version: u8) -> (u32, u32, u32) {
    if version == 2 {
        (tavern_bgs::service_hash::AUTHENTICATION_LISTENER_V2, 2, 3)
    } else {
        (tavern_bgs::service_hash::AUTHENTICATION_CLIENT_V1, 12, 13)
    }
}
...
async fn handle_verify_web_credentials(
    ...
    let consumed = tavern_db::service_tickets::consume(&state.db, ticket_str).await?;
    ...
    let ga_rows = tavern_db::game_accounts::find_by_account(&state.db, account_id).await?;

    session.game_accounts = ga_rows
        .iter()
        .map(|r| tavern_bgs::GameAccountHandle {
            id: r.id as u32,
    ...
    let lic_rows = tavern_db::account_licenses::find_by_account(&state.db, account_id).await?;
    let mut per_game: std::collections::HashMap<i64, Vec<tavern_bgs::AccountLicense>> =
        std::collections::HashMap::new();
    session.licenses = lic_rows
        .into_iter()
        .filter_map(|r| {
            let lic = tavern_bgs::AccountLicense {
    ...
    if session.game_account_info.iter().any(|g| g.is_banned) {
        ...
        let kick_body = DisconnectNotification {
            error_code: tavern_bgs::result_code::ERROR_GAME_ACCOUNT_BANNED,
    ...
    if session.game_account_info.iter().any(|g| g.is_suspended) {
        ...
        let kick_body = DisconnectNotification {
            error_code: tavern_bgs::result_code::ERROR_GAME_ACCOUNT_SUSPENDED,
    ...
    if all_expired {
        ...
        let kick_body = DisconnectNotification {
            error_code: tavern_bgs::result_code::ERROR_GAME_ACCOUNT_NO_TIME,
...
async fn handle_generate_sso_token(
    ...
    let req = if let Some(b) = body {
        tavern_bgs::GenerateSsoTokenRequest::decode(b)?
    } else {
        tavern_bgs::GenerateSsoTokenRequest::default()
    };
    ...
    let _ = tavern_db::bgs_sessions::insert(
        &state.db,
        &sso_id,
    ...
    .await;
    let response = tavern_bgs::GenerateSsoTokenResponse {
        sso_id: Some(sso_id),
...
async fn handle_generate_web_credentials(
    ...
    let req = if let Some(b) = body {
        tavern_bgs::GenerateWebCredentialsRequest::decode(b)?
    } else {
        tavern_bgs::GenerateWebCredentialsRequest::default()
    };
    ...
    let response = tavern_bgs::GenerateWebCredentialsResponse {
...
async fn handle_select_game_account(
    ...
    let req = if let Some(b) = body {
        tavern_bgs::SelectGameAccountRequest::decode(b)?
    } else {
        return Ok(Vec::new());
    };
...
async fn build_on_logon_complete(
    ...
    if error_code == 0 {
        ...
        for ga in &session.game_accounts {
            result.game_account_id.push(tavern_bgs::EntityId {
                high: 0x576f57, // "WoW"
                low: ga.id as u64,
            });
        }
...
async fn handle_game_utilities(
    ...
    match method_id {
        ...
        method_id::game_utilities::REGISTER_UTILITIES => {
            ...
            let response = tavern_bgs::ClientResponse {
                attribute: vec![tavern_bgs::Attribute {
                    name: "Param_ClientId".to_string(),
                    value: tavern_bgs::Variant {
...
fn build_account_response(
    ...
    let licenses_for = |ga_id: i64| -> Vec<tavern_bgs::AccountLicense> {
        session
    ...
    match method_id {
        method_id::account::RESOLVE_ACCOUNT => {
            ...
            let resp = ResolveAccountResponse {
                id: Some(tavern_bgs::AccountId { id: first_ga.id }),
            };
        ...
        method_id::account::GET_GAME_ACCOUNT_STATE => {
            ...
            let requested_id = body.and_then(|b| {
                tavern_bgs::GetGameAccountStateRequest::decode(b)
                    .ok()
...
async fn handle_restore_session(
        ...
        if active >= state.max_logins {
            ...
            let resp = serialize_frame(
                &tavern_bgs::Header {
                    ...
                    method_id: Some(2),
                    token: 0,
                    service_hash: Some(tavern_bgs::service_hash::SESSION_SERVICE),
                    is_response: Some(true),
                    status: Some(0),
                    ..Default::default()
                },
    ...
    if let Some(row) = tavern_db::bgs_sessions::find_valid(&state.db, &sso_id).await? {
        session.authenticated = true;
...
fn main() -> anyhow::Result<()> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async {
            ...
            tavern_observability::register_pool_gauge(db.clone());
            ...
            axum::serve(listener, app)
                .with_graceful_shutdown(tavern_observability::shutdown_signal())
                .await?;
