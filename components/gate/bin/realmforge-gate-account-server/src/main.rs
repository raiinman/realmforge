...
//! `realmforge-gate-account-server` binary.
//!
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tokio::runtime::Builder::new_multi_thread()
        ...
        .block_on(async {
            ...
            let meter_provider =
                realmforge_gate_observability::init("realmforge-gate-account-server");
            let config = Config::from_env()?;
            tracing::info!(bind = %config.bind_addr, "starting realmforge-gate-account-server");
            ...
            let state = realmforge_gate_account::app_state_with_region(
                pool,
                signing_key_pem,
                &config.region,
                &config.smtp_host,
            ...
            realmforge_gate_account::spawn_reaper(state.clone());
            ...
            let shutdown = realmforge_gate_observability::shutdown_signal();
