...
//!    parsed by the existing `realmforge_gate_bgs::frame` module.
//!
...
use realmforge_gate_bgs::frame::{FrameError, parse_frame};
...
async fn handle_tcp_connection<S>(mut stream: S, state: Arc<super::BgsState>) -> anyhow::Result<()>
where
